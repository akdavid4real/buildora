...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  if (!hydrated) {
    return (
      <div
        style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#f4f7f5' }}
      >
  ...
  return (
    <div className="app">
      <div
        className={`sidebar-overlay ${menu ? 'open' : ''}`}
      ...
      <aside className={`sidebar ${menu ? 'open' : ''}`} aria-label="Main sidebar navigation">
        ...
        <div className="site-switch">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span
              className={`badge ${!isApiMode ? 'draft' : ''}`}
              style={{
                fontSize: 10,
                padding: '2px 6px',
                background: isApiMode ? '#2a6a57' : '#ffffff20',
                color: 'white',
              }}
            >
              {isApiMode ? 'API' : 'Demo'}
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  borderRadius: 9,
                  textDecoration: 'none',
                  color: active ? 'white' : '#b9ccc5',
                  background: active ? '#ffffff13' : 'transparent',
                }}
              >
                <Icon size={17} /> {label}
              </Link>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            >
              <i className={`dot ${isApiMode ? 'live' : ''}`} />
              {isApiMode ? 'API Mode' : 'Offline Demo'}
            </span>
            <a
              className="btn btn-secondary"
              href={`/site/${state.site.siteSlug}`}
              target="_blank"
            ...
            <span
              className="avatar"
              title={currentUser?.email || (isApiMode ? 'API User' : 'Demo User')}
            >
              {initials}
            </span>
          </div>
        ...
        {apiError && (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span>
                <strong>API connection notice:</strong> {apiError} (fallback data loaded)
              </span>
            </div>
