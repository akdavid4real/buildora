...
import { BookOpen, Copy, Edit3, Eye, FileText, Globe2, Plus, Trash2 } from 'lucide-react';
...
export function ContentListView({ kind }: { kind: 'pages' | 'posts' }) {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (newId) router.push(`/dashboard/${kind}/${newId}`);
    } catch {}
  };
    ...
    <div className="content">
      ...
      {collection.length === 0 ? (
        <div className="card empty">
          ...
          <button className="btn btn-primary" onClick={handleCreate}>
            <Plus size={16} /> Create your first {isPages ? 'page' : 'post'}
          </button>
        </div>
      ) : (
        <div className="table-card">
          <table>
            <thead>
              <tr>
                <th>Title</th><th>URL Slug</th><th>Status</th><th>Last Updated</th><th style={{ textAlign: 'right' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {collection.map((item) => {
                  ...
                  <tr key={item.id}>
                    <td data-label="Title">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: 8, background: '#f0f5f2', color: '#174d3e', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                          {isPages ? <FileText size={16} /> : <BookOpen size={16} />}
                        </div>
                        <div>
                          <Link href={editHref} className="table-title">{item.title}</Link>
                          <span className="table-subtitle">
                            {isHomepage && <span className="badge" style={{ marginRight: 6, fontSize: 9, background: '#dcf0e7', color: '#15523f' }}>Homepage</span>}
                            {'authorName' in item && `By ${(item as PostItem).authorName} · `}ID: {item.id}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td data-label="URL"><code style={{ background: '#f3f6f4', padding: '3px 7px', borderRadius: 6, fontSize: 13, color: '#344c42' }}>/{isPages ? item.slug : `blog/${item.slug}`}</code></td>
                    <td data-label="Status"><span className={`badge ${item.status === 'DRAFT' ? 'draft' : ''}`}><i className={`dot ${item.status === 'PUBLISHED' ? 'live' : ''}`} style={{ marginRight: 5 }} />{item.status}</span></td>
                    <td data-label="Updated" style={{ color: '#6f7c77', fontSize: 13 }}>{item.updatedAt ? new Date(item.updatedAt).toLocaleDateString() : 'Recent'}</td>
