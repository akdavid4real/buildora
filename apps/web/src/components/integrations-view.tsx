...
import { BarChart3, CalendarDays, CheckCircle2, CreditCard, MessageCircle, Send } from 'lucide-react';
...
const ITEMS = [
  ...
  { id: 'mailchimp', name: 'Mailchimp', icon: Send, placeholder: 'Audience / signup URL', help: 'Newsletter destination or audience link.' },
  { id: 'paystack', name: 'Paystack', icon: CreditCard, placeholder: 'Payment page URL', help: 'Payment page for simple checkout links.' },
] as const;
          ...
          return (
            <section className="card" key={id}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 12 }}>
                ...
                <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 700 }}>
                  <input
                    ...
                    onChange={(e) => setConfig((prev) => ({ ...prev, [id]: { ...item, enabled: e.target.checked } }))}
                  />
              ...
              <input
                ...
                value={item.value || ''}
                onChange={(e) => setConfig((prev) => ({ ...prev, [id]: { ...item, value: e.target.value } }))}
              />
