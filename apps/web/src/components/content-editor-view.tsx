...
export function ContentEditorView({ kind, id }: { kind: 'pages' | 'posts'; id: string }) {
      ...
      <div className="content">
        <div className="card empty">
          ...
          <Link href={`/dashboard/${kind}`} className="btn btn-primary">
            <ArrowLeft size={16} />
            Back to {isPages ? 'Pages' : 'Blog posts'}
          </Link>
        </div>
      </div>
    );
  ...
  const patch = (patchData: Partial<PageItem & PostItem>, message?: string) => {
    if (isPages) {
      patchPage(id, patchData, message);
    } else {
      patchPost(id, patchData, message);
    }
  };
      ...
      <div className="editor-nav-bar">
        <div className="editor-nav-left">
          <Link href={`/dashboard/${kind}`} className="back-btn">
            <ArrowLeft size={16} />
            <span>Back to {isPages ? 'Pages' : 'Blog posts'}</span>
          </Link>
          ...
          {!isPages && (
            <div className="card" style={{ padding: 22 }}>
              ...
              <div className="field">
                ...
                <input
                  className="input"
                  value={(item as PostItem).authorName ?? ''}
              ...
              <div className="field" style={{ marginBottom: 0 }}>
                ...
                <textarea
                  className="textarea"
                  style={{ minHeight: 85 }}
                  placeholder="Short teaser shown on blog listings..."
                  value={(item as PostItem).excerpt ?? ''}
